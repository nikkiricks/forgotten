...
class TwitterAutomationService {
  ...
  async submitDeceasedAccountForm(submissionData, certificateBuffer, legalAuthBuffer = null) {
    try {
      ...
      // Navigate to X's deceased account form
      // Note: X/Twitter requires initial contact through help center, then email process
      const helpUrl = 'https://help.x.com/en/forms/account-access/deactivate-or-close-account/deactivate-account-for-deceased';

      await this.page.goto(helpUrl, {
        waitUntil: 'networkidle2',
        timeout: 30000
      });

      // Wait for the form to load
      await this.page.waitForSelector('form, .form-container, .support-form', { timeout: 10000 });

      // Fill out the X/Twitter deceased account form
      await this.fillTwitterForm(submissionData);

      // Handle file upload(s) - X requires death certificate and ID
      ...
      return {
        ...
        message: 'X/Twitter account removal request submitted successfully',
        estimatedProcessingTime: '7-14 business days',
        method: 'automated_form',
        requestType: 'removal' // X/Twitter only supports removal, not memorialization
      };
  ...
  async fillTwitterForm(data) {
    // X/Twitter deceased account form fields
    ...
    await this.fillFieldIfExists('input[name="your_first_name"]', data.firstName);
    await this.fillFieldIfExists('input[name="your_last_name"]', data.lastName);
    ...
    await this.fillFieldIfExists('input[name="your_email"]', data.contactEmail);
    ...
    await this.fillFieldIfExists('input[name="deceased_person_first_name"]', this.extractFirstName(data.deceasedName));
    await this.fillFieldIfExists('input[name="deceased_person_last_name"]', this.extractLastName(data.deceasedName));
    
    // Twitter username or profile URL
    await this.fillFieldIfExists('#twitter_username', this.extractUsername(data.twitterUrl));
    await this.fillFieldIfExists('input[name="twitter_username"]', this.extractUsername(data.twitterUrl));
    await this.fillFieldIfExists('input[name="username"]', this.extractUsername(data.twitterUrl));
    await this.fillFieldIfExists('input[name="account_username"]', this.extractUsername(data.twitterUrl));
    await this.fillFieldIfExists('input[name="twitter_handle"]', this.extractUsername(data.twitterUrl));
    await this.fillFieldIfExists('#profile_url', data.twitterUrl);
    await this.fillFieldIfExists('input[name="profile_url"]', data.twitterUrl);
    await this.fillFieldIfExists('input[name="account_url"]', data.twitterUrl);
    ...
    if (data.deceasedEmail) {
      ...
      await this.fillFieldIfExists('input[name="deceased_person_email"]', data.deceasedEmail);
    }
    ...
    if (data.dateOfDeath) {
      ...
      await this.fillFieldIfExists('input[name="death_date"]', data.dateOfDeath);
    }
    ...
    await this.fillFieldIfExists('textarea[name="details"]', description);
    await this.fillFieldIfExists('#additional_information', description);
    await this.fillFieldIfExists('textarea[name="message"]', description);
  ...
  async selectRelationship(relationship) {
      ...
      try {
        ...
        if (selectElement) {
          if (relationship === 'immediate-family') {
            // Try different X relationship values for immediate family
            const immediateOptions = ['immediate_family', 'spouse', 'parent', 'child', 'sibling', 'family_member', 'immediate family'];
            ...
            const extendedOptions = ['extended_family', 'other_family', 'family_member', 'extended family'];
            ...
            const legalOptions = ['legal_representative', 'executor', 'attorney', 'authorized_representative', 'legal representative'];
    ...
    if (relationship === 'immediate-family') {
      ...
      await this.clickElementWithText('Sibling');
      ...
      await this.clickElementWithText('Attorney');
    }
  }
  ...
  async fillFieldIfExists(selector, value) {
    try {
      ...
      console.log(`Twitter field ${selector} not found or couldn't be filled:`, error.message);
    }
  }
  ...
  buildDescription(data) {
    return `
Request for removal of deceased X/Twitter account.

Deceased Person: ${data.deceasedName}
X/Twitter Profile: ${data.twitterUrl}
...
Please process this account removal request according to X's policies for deceased accounts.
    `.trim();
  }
  ...
  async uploadFile(buffer, filename, fileType) {
    try {
      ...
      const tempPath = path.join('/tmp', `temp_tw_${Date.now()}_${filename}`);
      fs.writeFileSync(tempPath, buffer);

      // Common file upload selectors for X/Twitter forms
      ...
      if (fileInput) {
        ...
        try {
          ...
          console.log(`Twitter ${fileType} uploaded successfully`);
        } catch (uploadError) {
          console.warn(`Twitter ${fileType} upload confirmation not detected, but file may have uploaded:`, uploadError.message);
        }
      } else {
        console.warn(`Twitter ${fileType} file upload input not found on form`);
      }
    } catch (error) {
      console.error(`Error uploading Twitter ${fileType}:`, error);
    } finally {
      // Clean up temp file
      const tempPath = path.join('/tmp', `temp_tw_${Date.now()}_${filename}`);
  ...
  async submitForm() {
    try {
      ...
      if (submitButton) {
        console.log("Twitter submit button found and clicked");
        ...
        throw new Error('Submit button not found on Twitter form');
      }
    } catch (error) {
      console.error('Error submitting Twitter form:', error);
      throw error;
    }
  }
  ...
  async getConfirmationData() {
    try {
      // Look for confirmation message with X-specific selectors
      ...
      const confirmationId = caseNumberMatch ? caseNumberMatch[1] : `TW_AUTO_${Date.now()}`;

      return {
        id: confirmationId,
        message: confirmationText || 'Twitter form submitted successfully',
        timestamp: new Date().toISOString(),
        url: this.page.url()
      };
    } catch (error) {
      console.error('Error getting Twitter confirmation data:', error);
      return {
        id: `TW_AUTO_${Date.now()}`,
        message: 'Twitter form submitted - confirmation details not available',
        timestamp: new Date().toISOString(),
        url: this.page.url()
      };
    }
...
export default TwitterAutomationService;